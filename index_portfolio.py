...
"""
# Apply theme CSS based on current state
...
with st.sidebar:
    ...
    # Use return values instead of on_change lambdas
    ...
    display_choice = st.selectbox(
        "Display Mode", ["Showcase", "Minimal"],
        index=0 if st.session_state.display_mode == "Showcase" else 1,
        key="display_select"
    )
    ...
    page = st.radio(
        "Navigate",
        ["🏠 Home", "🙋 About Me", "🛠️ Skills", "📂 Projects", "📜 Certifications", "📬 Contact"],
        key="nav_radio"
    )

# Re-apply theme CSS after possible change (Streamlit reruns, but this keeps order explicit)
st.markdown(dark_css if st.session_state.theme == "Dark" else light_css, unsafe_allow_html=True)
st.markdown(base_css, unsafe_allow_html=True)
